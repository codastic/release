...
- `--dry-run`: _(optional)_ Outputs changes instead of writing to `CHANGELOG.md`.

- `--since`: _(optional)_ Limit search for pull requests to the given ISO date (e.g. `--since='2017-01-01'`).

- `--hide-reviewer`: _(optional)_ If set the reviewer will not be put into the `CHANGELOG.md`.

- `--hide-implementer` _(optional)_ If set the implementer will not be put into the `CHANGELOG.md`.

- `--interactive`: _(optional)_ If set every addition to the changelog has to be confirmed manually.

- `--link-commit`: _optional_ If set the commit hash in the output will be linked. Expects a template URL in the form of "http://example.com/:commit".

- `--all-commits`: (optional) If set all commits will be checked for merge messages and not only merge commits. Use this if you squash merge pull requests.
...
- `--push-build`: _(optional)_ Push the build in this ignored folder to the version branch.

- `--target-branch`: _(optional)_ (default: MAJOR.x) The branch where the release will be pushed to (e.g. `--target-branch='master'`).

- `--build-command`: _(optional)_ Run the build to be able to include the new version from `package.json` (e.g `--build-command='npm run build' ./"`). Also adds and commits the build files if they are not ignored.
