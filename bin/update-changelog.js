...
if (argv.help) {
  console.log(
    [
      ...
      '  --hide-implementer (optional) If set the implementer will not be put into the CHANGELOG.md.',
      ...
      '  --all-commits (optional) If set all commits will be checked for merge messages. Use this if you squash merge pull requests.'
...
function findCommitsInChangelog(callback) {
  ...
  input.on('error', () => {
    console.error(
      'Warning: %s does not exist, yet. Attempt to create file now.',
      changelogName,
    );
    callback([]);
  });
...
function resolveImplementers(pullRequests, callback) {
  function next(pos) {
    ...
    const child = exec(cmd, (error, stdout) => {
      ...
      if (
        argv['hide-reviewer']
        || pullRequest.implementer === pullRequest.reviewer
      ) {
...
function stringifyPullRequest({
  message, implementer, reviewer, commit
}) {
  const commitString = argv['link-commit']
    ? `[${commit}](${argv['link-commit'].replace(':commit', commit)})`
    : commit;

  const addInfo = [];
  if (!argv['hide-implementer']) {
    addInfo.push(`i: ${implementer}`);
  }
  if (!argv['hide-reviewer'] && reviewer) {
    addInfo.push(`r: ${reviewer}`);
  }
  addInfo.push(`c: ${commitString}`);

  return `- ${message} (${addInfo.join(', ')})\n`;
}

function filterPullRequestsInteractively(pullRequests) {
  return pullRequests.reduce(
    (promise, pullRequest) => promise.then(async (filteredPullRequests) => {
...
getPullRequests((pullRequests) => {
  findCommitsInChangelog((commits) => {
    ...
    const processPullRequest = () => {
      (argv.interactive
        ? filterPullRequestsInteractively(newPullRequests)
        : Promise.resolve(newPullRequests)
      ).then((finalPullRequests) => {
        if (finalPullRequests.length === 0) {
          console.error('Error: There are no new changes!');
          process.exit(1);
        }

        const changeString = finalPullRequests
          .map(stringifyPullRequest)
          .join('');
    ...
    if (argv['hide-implementer']) {
      ...
      resolveImplementers(newPullRequests, processPullRequest);
    }
  });
});
